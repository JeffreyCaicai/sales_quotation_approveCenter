...
- Create: `lib/imports/canonical-json.ts`
- Create: `lib/imports/import-lock.ts`
- Create: `lib/imports/multipart.ts`
- Create: `lib/imports/xlsx-container.ts`
- Create: `lib/imports/reconcile-pending-objects.ts`
...
Before writing any object, reserve the import job in state `uploading` with a unique attempt ID and a 15-minute lease. Finalization first commits `import_files` and the `uploaded` job state in an attempt-locked PostgreSQL transaction, then idempotently changes pending object tags to committed outside that transaction. If tagging is partial or interrupted, reconciliation sees the committed database references and only finishes tags; it never deletes referenced objects. Cleanup and expired-attempt recovery acquire the same attempt lock, recheck state/lease/references, and only delete owned pending versions for an uncommitted attempt. A conditional-put collision never deletes the pre-existing object. A production CLI and the Task 7 worker call `reconcilePendingObjects`, which skips active leases and safely retries expired attempts. PostgreSQL enforces that every `uploading` row has a non-null attempt ID and lease, while non-uploading rows have no active lease.
...
Parse `[Content_Types].xml`, package `_rels/.rels`, workbook XML, and workbook relationships as XML with exact OPC/spreadsheet namespaces. Require one internal package `officeDocument` relationship to `xl/workbook.xml`, one exact normal-XLSX content-type override, a valid workbook with at least one sheet that the XLSX parser can open, unique critical ZIP entries, no external/macro/VBA/embedded parts, at most 2,048 entries, and bounded per-entry/aggregate decompressed sizes and compression ratios.
