...
| Identifiers | IRIS Building ID is the required, permanent, unique, non-reusable building business identifier. ERP Building ID is an optional external mapping. Customer, Brand, Assignment, and Package identifiers are generated by the system on first import and included in later exports. Excel row numbers are never identifiers. |
...
- `buildings`: internal ID, immutable IRIS Building ID, nullable unique ERP Building ID, name, Building Type, Grade Resource, Area, City, CBD Area, Sub-District, Address, ERP link status, operational status, source metadata, timestamps.
...
- IRIS Building ID is supplied by the Building Team, remains unique and immutable, and is never reused. ERP Building ID is nullable and never replaces IRIS identity.
...
- Future CRM and ERP IDs are stored as external identifiers alongside internal IDs; they never replace internal primary keys or IRIS Building ID.
...
The building template requires IRIS Building ID, Building Name, and Operational Status. ERP Building ID is optional. The approved business attributes include Building Type, Grade Resource, Area, City, CBD Area, Sub-District, Address, and Data Source. ERP Link Status is derived by the system. The detailed identity, reconciliation, validation, and template rules are defined in `2026-07-11-building-identity-erp-mapping-design.md` and require an explicit template-version change.
...
- building prices: IRIS Building ID and price;
...
- Reject duplicate IRIS Building IDs, generated IDs, and exact duplicate assignment rows.
