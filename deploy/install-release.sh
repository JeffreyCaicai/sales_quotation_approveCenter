...
# Load the trusted root-owned environment without printing it.
set -a
...
# Keep current plus the two most recent prior releases.
mapfile -t stale < <(find "$releases" -mindepth 1 -maxdepth 1 -type d -regextype posix-extended \
  -regex '.*/[0-9a-f]{40}' -printf '%T@ %p\n' | sort -rn | tail -n +4 | cut -d' ' -f2-)
for old in "${stale[@]}"; do rm -rf -- "$old"; done
trap - EXIT
