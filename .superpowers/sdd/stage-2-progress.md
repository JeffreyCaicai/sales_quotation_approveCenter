...
IRIS Task 6: complete with reviewed central IRIS-to-UUID Rate Card reference resolution, isolated native lifecycle fixtures, executable PGlite lifecycle, and deterministic 5,000-row parse/validate/diff coverage. The resolver is the boundary for a later full Rate Card publisher; this task does not claim that publisher exists. Native PostgreSQL integration remains unverified locally: `localhost:55432` rejected the required run with `EPERM`, and no Docker/container runtime is available. This gap includes native advisory-lock and transactional publication behavior; it is not represented as passing PGlite coverage.
